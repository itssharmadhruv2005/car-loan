// INR Currency Formatter (Indian numbering system: Lakhs/Crores)
...
// Format numbers nicely without currency symbol
...
// PMT calculator to find monthly EMI
...
// Bisection method to solve for RATE function
export function findRate(nper, pmt, pv, fv = 0, type = 0) {
...
// Date helper to increment month labels
export function getMonthLabel(startMonthStr, monthIndex) {
  try {
...
// Calculations Engine run in-browser for zero lag
export function runCalculations(inputs) {
  ...
  const swpAmount = isAutoLinked ? monthlyEmi : (Number(inputs.swp_amount) || monthlyEmi);
  ...
  return {
    ...
    schedule: schedule.slice(0, tenureMonths),
